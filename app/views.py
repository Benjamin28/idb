...
def api_model(model):
    ...
    if len(req_str) == 0:
        ...
        str_dict = {'orderBy': None, 'order': 'asc',
                    'page': 1, 'limit': NUM_PER_PAGE}
        ...
        criteria = str_dict['orderBy'] if str_dict['orderBy'] in m.attributes(
        ) else 'id'
        ...
        if model == 'agencies' and str_dict['agencyType'] != None:
            if type(str_dict['agencyType']) is list:
                ...
                query_list = query_list.filter_by(
                    agencyType=str_dict['agencyType'])
            ...
            if type(str_dict['countryCode']) is list:
                ...
                query_list = query_list.filter(
                    Location.countryCode.in_(my_list))
            else:
                query_list = query_list.filter_by(
                    countryCode=str_dict['countryCode'])
            ...
            if type(str_dict['typeName']) is list:
                ...
                query_list = query_list.filter_by(
                    typeName=str_dict['typeName'])

        if(str_dict['order'] == 'desc'):
            query_list = query_list.order_by(desc(criteria)).paginate(
                page_num, int(str_dict['limit']), False).items
        else:
            query_list = query_list.order_by(criteria).paginate(
                page_num, int(str_dict['limit']), False).items
        if query_list == []:
...
def api_search():
    ...
    # Model list
    MODELS = [[Agency, "agencies"], [Launch, "launches"],
              [Location, "locations"], [Mission, "missions"]]
    ...
    for modl in MODELS:
        ...
        if 'term' in req_str:
            ...
            and_str = "and_" + m_str
            or_str = "or_" + m_str
            return_dict["and_search"][and_str] = search_and(
                m, search_terms.replace("+", " "))
                ...
                if not "_sa_instance_state" in l_key and l_key != "id" and (l_term in l_key or l_term in l_value):
                    ...
                    if l_term in l_value:
                        highlight_list.append(
                            key + " : " + highlight_word(value, term))
                    else:
                        highlight_list.append(
                            key + " : " + highlight_word(key, term))
                    counter = counter + 1
            if is_here:
            ...
            for key, value in item.__dict__.items():
                ...
                for word in terms_list:
                    ...
                    if not "_sa_instance_state" in l_key and l_key != "id" and (l_word in l_key or l_word in l_value):
                        exists = True
                        highlight_list.append(
                            highlight_words((key, value), word))
                        counter = counter + 1
            if exists:
                t["highlight_list"] = highlight_list
                results_list += [t]
...
def highlight_word(word, term):
    ...
    if word_loc >= 0:
        highlight_word = word[:word_loc] + '<span class="highlight"><strong>' + word[word_loc:(
            word_loc + len(term))] + '</strong></span>' + word[(word_loc + len(term)):]
