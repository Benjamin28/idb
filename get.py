...
# TODO: Address what to do with duplicates. Launch gets updated as ID
# increases, agency doesnt, mission has the most updated on the first
...
def create_agency_types():
    agencytype_response = requests.get(
...
def create_launch_statuses():
    launchstatus_response = requests.get(
...
def create_agency():
    ...
    agency_response = requests.get(
        "https://launchlibrary.net/1.2/agency?limit=173")
    agency_json = json.loads(agency_response.text)

    for agency in agency_json["agencies"]:
        inner_agency_response = requests.get(
            "https://launchlibrary.net/1.2/agency/" + str(agency["id"]))
        inner_agency_json = json.loads(inner_agency_response.text)

        agency_info = inner_agency_json["agencies"][0]
        duplicate_check = db.session.query(Agency).filter_by(
            name=agency_info["name"]).first()
        if duplicate_check is None:
            assert agency_info["name"] != ""
...
def create_launch():
    ...
    launch_response = requests.get(
        "https://launchlibrary.net/1.2/launch?limit=1088")
    launch_json = json.loads(launch_response.text)

    for launch in launch_json["launches"]:
        inner_launch_response = requests.get(
            "https://launchlibrary.net/1.2/launch/" + str(launch["id"]))
        inner_launch_json = json.loads(inner_launch_response.text)

        launch_info = inner_launch_json["launches"][0]

        current_videoUrl = launch_info["vidURLs"][0] if len(
            launch_info["vidURLs"]) > 0 else "None"
        current_launchPad = launch_info["location"]["pads"][0]["name"] if len(
            launch_info["location"]["pads"]) > 0 else "None"

        # LOCATION (Get the one location associated with this launch)
        current_location = launch_info["location"]["name"]
        if current_location != "":
            related_location = db.session.query(
                Location).filter_by(name=current_location).first()

        duplicate_check = db.session.query(Launch).filter_by(
            name=launch_info["name"]).first()
        if duplicate_check is None:
            ...
            current_launch = Launch(name=launch_info["name"], windowStart=launch_info["windowstart"], windowEnd=launch_info["windowend"],
                                    videoUrl=current_videoUrl, launchPad=current_launchPad, rocket=launch_info[
                                        "rocket"]["name"],
            ...
            for agency in launch_info["location"]["pads"][0]["agencies"]:
                related_agency = db.session.query(
                    Agency).filter_by(name=agency["name"]).first()
...
def create_mission():
    ...
    mission_response = requests.get(
        "https://launchlibrary.net/1.2/mission?limit=495")
    mission_json = json.loads(mission_response.text)

    for mission in mission_json["missions"]:
        inner_mission_response = requests.get(
            "https://launchlibrary.net/1.2/mission/" + str(mission["id"]))
        ...
        current_launch = mission_info["launch"]["name"] if len(
            mission_info["launch"]) > 0 else "None"
        if current_launch != "":
            related_launch = db.session.query(
                Launch).filter_by(name=current_launch).first()

        duplicate_check = db.session.query(Mission).filter_by(
            name=mission_info["name"]).first()
        if duplicate_check is None:
            assert mission_info["name"] != ""
